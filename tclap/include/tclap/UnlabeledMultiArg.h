 ...
 *  
 *Modified by David Cleres And Yann Dubois to work with vectors
 *****************************************************************************/ 
...
class UnlabeledMultiArg : public MultiArg<T>
{
    ...
    using MultiArg<T>::_constraint;
    using MultiArg<T>::_values;
	...
	public:
		...
		UnlabeledMultiArg( const std::string& name,
						   const std::string& desc,
						   bool req,
						   VectorConstraintTCLAP* constraint,
						   bool ignoreable = false,
						   Visitor* v = NULL );
...
bool UnlabeledMultiArg<T>::processArg(int *i, std::vector<std::string>& args) 
{
	...
	*/
    ...
    //on veut faire test sur vecteur
    if ( (unsigned int)(*i) == (args.size()-1) && _constraint != NULL )
        if ( ! _constraint->check( _values ) )
            throw( CmdLineParseException( "Value does not meet constraint: " +
